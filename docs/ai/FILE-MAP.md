...
| Path | Role | Risk | Edit protocol | Notes |
|---|---|---|---|---|
| `src/games/dnd/journal/DndJournalRoute.tsx` | `/dnd/journal` orchestrator: page list, selection, realtime sync, computes `isEditor` | medium | before-any-change | Public read (no login); account only used to check editor identity |
| `src/games/dnd/journal/{types,constants,mappers}.ts` | Journal page/image contract, type↔category taxonomy, row↔app mapping, `DND_JOURNAL_OWNER_AUTH_USER_ID` | high | supabase-edit-protocol | Must stay in sync with the RenaCompanion iPad app's `JournalPage.swift`/`JournalImage.swift` and with the SQL contract; owner id must match the SQL RLS literal |
...
| `src/games/dnd/journal/api/*` | Supabase CRUD for pages/images | high | supabase-edit-protocol | Soft-delete only; no hard DELETE policy; writes rejected by RLS unless caller is the owner account; image URLs are public (`getPublicUrl`), not signed; `updateJournalPage` reconciles `bodyMarkdown` via `merge.ts` when a baseline is given |
...
| `src/games/dnd/journal/supabase/dnd_journal.sql` | `dnd_journal_pages`/`dnd_journal_images` schema, RLS, `dnd-journal-images` public storage bucket | **critical** | supabase-edit-protocol | **Applied** to the live project; `select` is `to public` (no login) on both tables, single hardcoded-owner-id write; images bucket is public with no SELECT policy (Supabase advisor: avoids file-listing exposure); see `DECISIONS.md` 2026-07-31 |
