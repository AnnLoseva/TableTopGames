...
| `src/app/dnd/journal/page.tsx` | `/dnd/journal` route | low | before-any-change | Thin wrapper; new isolated D&D domain |
...
| `src/games/pathfinder2/sheet/rules-data-source.ts` | Build-time adapter from raw Pathfinder JSON to the runtime catalog | medium | before-any-change | Imported by the catalog generator; keep raw JSON out of client modules |
| `src/games/pathfinder2/sheet/data/rules-catalog-client.ts` | Verified client loader for Pathfinder runtime chunks | medium | before-any-change | Supabase first in production, generated local fallback |
| `src/platform/rules/catalog-manifest.ts` | Shared manifest validation, fetch and SHA-256 verification | high | supabase-edit-protocol | Release paths must remain relative and content-addressed |
...
| `src/games/pathfinder2/Rules/*.json` | Owner-provided Pathfinder ancestries, backgrounds, classes and feats | high | before-any-change | Preserve source/version/license metadata; keep schema changes behind `rules-data-source.ts` |
| `src/games/pathfinder2/Rules/catalogs/*.json` | Normalized canonical catalog documents (schemaVersion 1) | high | before-any-change | Generated by normalizer; do not edit manually |
| `src/games/pathfinder2/scripts/normalize-rules.ts` | Deterministic normalizer: raw JSON → canonical catalogs | medium | before-any-change | Run after owner-provided raw JSON updates |

## D&D domain (`src/games/dnd/*`)

| Path | Role | Risk | Edit protocol | Notes |
|---|---|---|---|---|
| `src/games/dnd/journal/DndJournalRoute.tsx` | `/dnd/journal` orchestrator: three-panel shell, selection/history, page+folder Realtime, computes `isEditor` | medium | before-any-change | Public read (no login); account only used to check editor identity |
| `src/games/dnd/journal/{types,constants,mappers,folder-tree}.ts` | Page/folder/image contract, nested-tree validation, taxonomy, row↔app mapping, `DND_JOURNAL_OWNER_AUTH_USER_ID` | high | supabase-edit-protocol | Must stay in sync with the RenaCompanion app's `JournalPage.swift`/`JournalFolder.swift`/`JournalImage.swift` and SQL; owner id must match the RLS literal |
| `src/games/dnd/journal/merge.ts` | Paragraph-level `bodyMarkdown` conflict merge (canonical policy) | high | supabase-edit-protocol | Pure, unit-verified; any other writer (iPad app) must replicate this exact algorithm or conflicts lose data on that path; see `DECISIONS.md` 2026-07-31 |
| `src/games/dnd/journal/api/*` | Supabase CRUD for pages, nested folders and images | high | supabase-edit-protocol | Soft-delete only; folder delete moves direct pages/children to section root before writing a tombstone; image URLs are public; body updates reconcile through `merge.ts` |
...
| `src/features/characters_map/components/{RelationshipPanel,AddCharacterModal,AddRelationshipModal}.tsx` | View/edit UI for a selected edge and creation forms | low | before-any-change | Edit affordances hidden unless `isEditor` |
...
| `src/features/characters_map/timeline.ts` | Pure timeline resolution: character kind/alive and relationship active/label/color/description at a given year, plus timeline bounds/marks | high | before-any-change | The only place that folds `events` chronologically — UI must go through it, never read `events` directly |
...
| `src/games/vampires/lib/rules-catalog-client.ts` | VTM versioned rules loader | high | rules-data + supabase-edit-protocol | Checked-in direct JSON remains final fallback |
...
| `src/games/vampires/supabase/rules_catalog_storage.sql` | read-only VTM/PF rule delivery buckets | **critical** | supabase-edit-protocol | No browser write policies |
...
| `public/vampires/rules-catalog-loader.js` | legacy verified rules delivery + fallback | high | legacy-edit-protocol + rules-data | Does not change sheet persistence |
...
| `scripts/generate-rule-catalogs.ts` | deterministic VTM/PF manifest + runtime release generator | medium | rules-data subsystem | Runs before dev/build; output is ignored |
| `scripts/publish-rule-catalogs.ts` | manifest-last Supabase Storage publisher and verifier | high | supabase-edit-protocol | Requires server-side publish key |
