...
| `src/games/dnd/journal/api/*` | Supabase CRUD for pages/images | high | supabase-edit-protocol | Soft-delete only; no hard DELETE policy; writes rejected by RLS unless caller is the owner account; image URLs are public (`getPublicUrl`), not signed |
