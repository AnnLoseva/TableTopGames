...
| Лор и таблицы | category tree/list + entry editor/preview + links; точные колонки не подтверждены | categories, entries, links, visibility | draft private; explicit publish | `games/vampires/modules/reference` renderer/catalog patterns, JournalEditor for rich text |
