...
Registered as `actors` in `games/vampires/modules/master-console/contributions.ts`.
...
Does **not** import `GameTable.tsx`. Reuses `games/vampires/modules/table` character sheet URL
helper, scene fetch for active scene, and master action log API.

## Layout

```text
games/vampires/modules/actors/
