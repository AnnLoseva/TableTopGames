...
function findPotentiallyUnusedFiles(files: FileInfo[]) {
  ...
  return files
    ...
    .filter(file => !file.path.startsWith('tooling/'))
    .filter(file => !file.path.startsWith('src/games/vampires/scripts/'))
...
async function main() {
  ...
  console.log('Heuristic: source/style files outside src/app, tooling/game scripts, and public with no local import found.')
