...
import attributeFiltersCatalog from '../Rules/catalogs/attribute-filters.json'
...
type RawAttributeFilterCatalog = {
  ancestries: Array<{
    id: string
    abilityBoosts: Pathfinder2AttributeKey[]
    abilityFlaw: Pathfinder2AttributeKey | null
  }>
  classes: Array<{
    id: string
...
export function getPathfinder2RulesCatalog(): Pathfinder2RulesCatalog {
  ...
  const attributeFilters = attributeFiltersCatalog as RawAttributeFilterCatalog
  const ancestryAttributeFilters = new Map(
    attributeFilters.ancestries.map(entry => [entry.id, entry]),
  )
  const classAttributeFilters = new Map(
    attributeFilters.classes.map(entry => [entry.id, entry]),
  )
  ...
  const ancestries: Pathfinder2AncestryRule[] = safeArray(ancestryDocument.ancestries)
    .map((ancestry, ancestryIndex) => {
      ...
      const attributeFilter = ancestryAttributeFilters.get(ancestryId)
      const abilityBoosts = safeArray(ancestry.abilityBoosts)
      return {
        id: ancestryId,
        name: ancestryName,
        ...
        size: ancestry.size ?? '',
        abilityBoosts: abilityBoosts.length
          ? abilityBoosts
          : attributeFilter?.abilityBoosts ?? [],
        abilityFlaw: ancestry.abilityFlaw !== undefined
          ? ancestry.abilityFlaw
          : attributeFilter?.abilityFlaw ?? null,
  ...
  const classes: Pathfinder2ClassRule[] = safeArray(classDocument.classes)
    .map((characterClass, index) => {
      ...
      return {
        ...
        keyAbilities: parseKeyAbilities(characterClass.keyAbility).length
          ? parseKeyAbilities(characterClass.keyAbility)
          : classAttributeFilters.get(classId)?.keyAbilities ?? [],
