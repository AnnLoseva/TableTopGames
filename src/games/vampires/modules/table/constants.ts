export * from './constants/roll-traits'

export const CHARACTERS = 'characters'
export const TABLE_ROLLS = 'table_rolls'
export const TABLE_CHAT_MESSAGES = 'table_chat_messages'
export const TABLE_IMAGES = 'table_images'
export const TABLE_SCENES = 'table_scenes'
export const TABLE_SCENE_FOLDERS = 'table_scene_folders'
export const TABLE_SCENE_MUSIC = 'table_scene_music'
export const TABLE_TOKENS = 'table_tokens'
export const TABLE_CHARACTER_CONTROLLERS = 'table_character_controllers'
export const TABLE_IMAGE_BUCKET = 'table-images'
export const ROOT_LAYER_DROP_ID = '__root__'
export const MASTER_PASSWORD_KEY = 'vtm-table-master-password'
export const DEFAULT_SCENE_NAME = 'Основная сцена'
export const DEFAULT_SCENE_WIDTH = 1920
export const DEFAULT_SCENE_HEIGHT = 1080
export const DEFAULT_TOKEN_SIZE = 160
/** Container z-index that keeps every character token above every media layer. */
export const TOKEN_LAYER_Z = 100000
